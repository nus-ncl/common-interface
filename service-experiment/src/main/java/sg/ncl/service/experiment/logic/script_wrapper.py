...
# Default server
XMLRPC_SERVER   = "boss.test.ncl.sg"
XMLRPC_PORT   = 3069
...
API = {
    ...
    "resetNetwork"	: { "func" : "resetNetwork",
                            "help" : "Restart experiment vlans" },
};
...
def do_method(module, method, params):
    ...
    server = xmlrpclib.ServerProxy(URI, SSL_Transport(ctx), verbose=debug)
    ...
    # 
    if len(response["output"]) and method != "extra_nodes":
...
class node_list:
    ...
    def apply(self):
        try:
            opts, req_args = getopt.getopt(self.argv, "cpPvhHme:", [ "help" ]);
            pass
        except getopt.error as e:
            print(e.args[0])
            self.usage();
        ...
        for opt, val in opts:
            if opt == "--help":
                ...
                print("Unsupported operation, sorry")
                exit(1)
                which = "pphysauxtype";
                pass
            elif opt == "-m":
    ...
    # If the experiment is has extra nodes (is traditionally containerized)
    # print the extra nodes.
        if which == "container":
            rval,response = do_method("experiment", "extra_nodes", params);
        ...
        is_pnode = re.compile("pnode-\\d\\d\\d\\d$");
        # Print the nodes as requested.  Note that containerized experiments
        # drop through here, printing embedded pnodes and ignoring the ones
        # holding VMs etc.
    ...
    def usage(self):
        ...
        print("     -p   - Print physical (DETER database) names (default)");
        ...
        print("     -c   - Print container VMs and physical nodes");
        #print("     -H   - Like -h, but include node auxtypes (NOTE: unsupported on DETER)");
        wrapperoptions();
        return
    pass
...
#
# resetNetwork - remove and restore all experiment vlans
#
class resetNetwork:
    def __init__(self, argv=None):
        self.argv = argv;
    ...
    def apply(self):
        try:
            opts, req_args = getopt.getopt(self.argv, "-v:", [ "help" ]);
            pass
        except getopt.error as e:
            print(e.args[0])
            self.usage();
        ...
        rval,response = do_method("experiment", "resetNetwork", params);
        return rval;

    def usage(self):
        print("template_stoprun [-v level] pid eid]");
        print("where:");
